...
class Transaksi {
    ...
    public function getTransactionById($id) {
        $stmt = $this->db->prepare("SELECT t_booking.*, t_lapangan.nama_lapangan, t_lapangan.jenis_lapangan, t_pengguna.nama
                                  FROM t_booking
                                  JOIN t_lapangan ON t_booking.id_lapangan = t_lapangan.id_lapangan
                                  JOIN t_pengguna ON t_booking.id_pengguna = t_pengguna.id_pengguna WHERE t_booking.id_booking = ?");
        $stmt->execute([$id]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);  // Ambil semua hasil
    }    
}
?>